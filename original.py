    ...
    for x in range(1, input_img.shape[0]-1):
        for y in range(1, input_img.shape[1]-1):
            ...
            if (largest-smallest > threshold):
                ...
                #check if corner
                if (((input_img[x-1, y-1] > threshold and input_img[x+1, y+1] < threshold) or (input_img[x-1, y-1] < threshold and input_img[x+1, y+1] > threshold)) and
                    ((input_img[x+1, y-1] > threshold and input_img[x-1, y+1] < threshold) or (input_img[x+1, y-1] < threshold and input_img[x-1, y+1] > threshold)) and
                    ((input_img[x, y-1] > threshold and input_img[x, y+1] < threshold) or (input_img[x, y-1] < threshold and input_img[x, y+1] > threshold)) and
                    ((input_img[x+1, y] > threshold and input_img[x-1, y] < threshold) or (input_img[x+1, y] < threshold and input_img[x-1, y] > threshold))):
                        output_img[x,y] = input_image[x,y]
